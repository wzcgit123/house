...
<DIV class=search> 【欢迎:${user.name}】   <LABEL class="ui-green searchs"><a href="selectTypeAll2" title="">发布房屋信息</a></LABEL>||  <LABEL class="ui-green searchs"><a href="/selectHouse" title="">管理房屋</a></LABEL>
...
<c:forEach items="${pageInfo.list}" var="h" >
  <TR>
    ...
    <TD class=house-type><LABEL class=ui-green><INPUT onclick=location.href="/getUpHouse?id=${h.id}" value="修    改" type=button name=search></LABEL></TD>
    <TD class=house-price><LABEL class=ui-green><INPUT onclick=location.href="/delHouse?id=${h.id}" value="删    除" type=button name=search>
    </LABEL></TD>
  </TR>
</c:forEach>
  </TBODY></TABLE></DIV>
